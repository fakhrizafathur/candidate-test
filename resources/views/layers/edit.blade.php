                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('suppliers.layups.layers.update', [$supplier, $layup, $layer]) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" id="layer_order" name="layer_order" value="{{ old('layer_order', $layer->layer_order) }}" min="1" required
                        ...
                        <div>
                            ...
                            <input type="number" id="thickness" name="thickness" value="{{ old('thickness', $layer->thickness) }}" step="0.01" min="0.01" required
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            @error('thickness')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" id="width" name="width" value="{{ old('width', $layer->width) }}" step="0.01" min="0.01" required
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            @error('width')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="angle" name="angle" required
                                ...
                                <option value="0" {{ old('angle', $layer->angle) == '0' ? 'selected' : '' }}>0°</option>
                                <option value="45" {{ old('angle', $layer->angle) == '45' ? 'selected' : '' }}>45°</option>
                                <option value="90" {{ old('angle', $layer->angle) == '90' ? 'selected' : '' }}>90°</option>
                            </select>
                            @error('angle')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="flex space-x-4">
                            ...
                            <a href="{{ route('suppliers.layups.layers.show', [$supplier, $layup, $layer]) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                                {{ __('Cancel') }}
                            </a>
                        </div>
                    </form>
